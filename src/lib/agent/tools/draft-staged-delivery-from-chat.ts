...
async function resolveItem(
  ...
  // ── Units staged ──────────────────────────────────────────────────────────
  let unitsDraft: UnitsStagedDraft;
  ...
  if (
    ...
    if (unitsDraft.resolved_units > availableUnits) {
      ...
      warnings.push(
        `Only ${availableUnits} available units for ${productLabel} / ${treatLabel}${sizeLabel} / ${packageTypeDraft.resolved_package_type} — this staged delivery would exceed available inventory by ${over} units.`
      );
    }
  }
...
async function logCall(
  ...
  const { data, error } = await serviceClient
    ...
    .insert({
      ...
      message_id: null,
      user_id: userId,
      tool_name: "draft_staged_delivery_from_chat",
      input_json: input,
      output_json: output,
      status,
...
export function makeDraftStagedDeliveryFromChatTool(
  userClient: SupabaseClient,
  serviceClient: SupabaseClient,
  userId: string,
  threadId: string,
  ...
  return tool<ToolInput, ToolOutput>({
    description:
      "Extracts a staged delivery from the user's natural language, resolves all fields " +
      ...
      "A staged delivery is product set aside for a customer that has NOT yet been officially delivered. " +
      "Supports multiple line items in a single call. " +
    ...
    inputSchema: jsonSchema<ToolInput>({
      ...
      properties: {
        ...
        items: {
          type: "array",
          description:
            "One entry per staged delivery line. Multi-line staged deliveries are fully supported.",
          items: {
            type: "object",
            properties: {
              productName: {
              ...
              units: {
                type: "number",
                description: "Number of units to stage. Must be a positive whole number.",
              },
              seedSize: {
                type: "string",
                description:
        ...
        notes: {
          type: "string",
          description: "Any notes or comments the user mentioned for this staged delivery.",
        },
      },
      required: ["customerName", "stagedDate", "items"],
      additionalProperties: false,
    }),

    execute: async (input: ToolInput): Promise<ToolOutput> => {
      // ── 1. Staged date ──────────────────────────────────────────────────
      const dateResolution = resolveAgentDate(userMessage, input.stagedDate);
      const stagedDateDraft: StagedDateDraft = {
        input: input.stagedDate,
      ...
      if (stagedDateDraft.status !== "resolved") {
        missingFields.push("staged date");
      }
      ...
      for (const ri of resolvedItems) {
        ...
        if (ri.units_staged.status !== "resolved") {
          missingFields.push(
            ri.units_staged.status === "invalid"
