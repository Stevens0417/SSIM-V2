...
import type { ReplantViewRow } from "@/services/replants.service";
...
import styles from "./ThisSeasonReplantsTable.module.css";
...
interface Props {
  ...
  onDelete: (replantId: string) => void;
  onUpdate: (
    replantId: string,
    updates: {
      replant_date: string;
...
export default function ThisSeasonReplantsTable({
  rows,
  loading,
  error,
  pricingOptions,
  ...
  const saveEdit = () => {
    ...
    if (!editForm.units_replanted || editForm.units_replanted <= 0) {
      setEditError("Units must be greater than 0");
      return;
    }
    if (!editForm.replant_date) {
      setEditError("Replant date is required");
      return;
    }

    onUpdate(editingId, {
      replant_date: editForm.replant_date,
  ...
  const handlePrintRow = (row: ReplantViewRow) => {
    ...
    const printItems: ReplantPrintItem[] = [
      {
  ...
  if (loading) return <div className={styles.status}>Loading replants…</div>;
    ...
    <div>
      ...
      <div className={styles.searchBar}>
        <input
          ...
          type="text"
          placeholder="Filter by customer, product, treatment, or notes…"
          value={searchTerm}
      ...
      {filtered.length === 0 ? (
        <div className={styles.status}>
          {rows.length === 0
            ? "No replants found for this season."
            : "No matching replants."}
        </div>
      ) : (
              ...
              {filtered.map((r) => (
                <tr key={r.replant_id}>
                  <td className={styles.replantId}>
                    {r.replant_id.slice(0, 8)}
                  </td>
                  <td>{r.replant_date}</td>
                    ...
                    <span className={styles.actionGroup}>
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (
                            window.confirm(
                              `Delete this replant for ${r.customer_name}?`
                            )
                          ) {
                            onDelete(r.replant_id);
                          }
                        }}
                      >
                        Delete
        ...
        <div className={styles.modalOverlay} onClick={cancelEdit}>
          ...
          >
            <div className={styles.modalHeader}>
              <h3 className={styles.modalTitle}>Edit Replant</h3>
            ...
            <div className={styles.modalBody}>
              ...
              <div className={styles.formField}>
                <label className={styles.formLabel}>Replant Date</label>
                <input
                  type="date"
                  className={styles.formInput}
                  value={editForm.replant_date}
                  onChange={(e) =>
                    setEditForm({ ...editForm, replant_date: e.target.value })
                  }
                />
              </div>
              ...
              <div className={styles.formField}>
                <label className={styles.formLabel}>Units Replanted</label>
                <input
                  type="number"
                  className={styles.formInput}
                  value={editForm.units_replanted || ""}
                  onChange={(e) =>
                    setEditForm({
                      ...editForm,
                      units_replanted: parseInt(e.target.value) || 0,
                    })
                  }
                  min={1}
                />
            ...
            <div className={styles.modalFooter}>
              ...
              <button
                className={styles.printBtn}
                onClick={() => {
                  const row = rows.find((r) => r.replant_id === editingId);
