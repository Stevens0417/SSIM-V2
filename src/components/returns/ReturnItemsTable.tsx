...
import styles from "./ReturnItemsTable.module.css";
...
export function createEmptyReturnItem(): ReturnItem {
  return {
...
export default function ReturnItemsTable({
  ...
  const addRow = () => {
    if (disabled) return;
    onChange([...items, createEmptyReturnItem()]);
  };

  const removeRow = (index: number) => {
    if (disabled) return;
    if (items.length <= 1) {
      onChange([createEmptyReturnItem()]);
      return;
    }
        ...
        <table className={styles.table}>
          ...
          <thead>
            <tr>
              ...
              <th className={styles.colNum}>Units Returned</th>
        ...
        {items.map((item, i) => {
          ...
          return (
            <div key={item.id} className={styles.card}>
              ...
              <div className={styles.cardField}>
                <label className={styles.cardLabel}>Units Returned</label>
                <input
