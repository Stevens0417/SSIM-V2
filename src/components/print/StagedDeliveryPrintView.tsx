...
export interface StagedDeliveryPrintItem {
  product: string;
  treatment: string;
  units: number;
}

export interface StagedDeliveryPrintCustomer {
  name: string;
  farmName: string;
  tsaNumber: string;
  phone: string;
...
export default function StagedDeliveryPrintView({
  stagedDate,
  customer,
  items,
  notes,
  ...
  const emptyRowCount = Math.max(0, MAX_PRINT_ROWS - items.length);
